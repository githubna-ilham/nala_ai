...
**Embedding** adalah representasi teks sebagai **vektor** — daftar angka dengan panjang tetap (di sini 768 angka desimal) — yang menangkap makna semantik teks itu, bukan cuma susunan hurufnya. Teks yang maknanya mirip akan menghasilkan vektor yang "berdekatan" secara matematis, sementara teks yang maknanya jauh berbeda menghasilkan vektor yang "berjauhan" — inilah dasar dari pencarian berbasis makna yang dibahas lebih detail di Module 12 (Bagian 1).
...
**Keuntungan embedding** dibanding pencocokan kata kunci murni: menangkap makna semantik (bukan cuma pencocokan kata), memungkinkan pencarian lebih akurat dan kontekstual, bisa mendeteksi sinonim dan variasi bahasa — jadi dasar untuk retrieval berbasis kemiripan dalam RAG (Module 9 Bagian 4).
...
**Prasyarat**: sudah menyelesaikan Module 10 (Setup Knowledge Base & Data Dasar Awal). Container `ollama`+`api` masih berjalan — module ini belum butuh `opensearch`/`airflow`.

**Langkah 1 — Pull model `nomic-embed-text`**

Belum pernah dipakai sejak Module 1-10 — di-pull tepat sekarang karena baru sekarang dibutuhkan. Pull-nya lewat **`docker compose exec`** (di dalam container), sama seperti `llama3.2:3b` di Module 1-4:

```bash
docker compose exec ollama ollama pull nomic-embed-text
```

`nomic-embed-text` tidak pakai tag, sesuai default parameter `model` di `app/embeddings.py` (Bagian 4).

```bash
docker compose exec ollama ollama list
```

`llama3.2:3b` (Module 7) dan `nomic-embed-text` harus muncul berdua di daftar.
...
```
Buat app/embeddings.py dengan fungsi embed_text() (Module 11, Tahap A).
...
- **`Internal Server Error` saat memanggil `embed_text()` atau saat ingest**: cek dulu model mana yang belum ter-pull (`docker compose exec ollama ollama list`) — module ini butuh dua model sekaligus: `llama3.2:3b` (dari Module 7) dan `nomic-embed-text` (Bagian 3). Kalau salah satu tidak muncul, ulangi langkah pull yang sesuai.
