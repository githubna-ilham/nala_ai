...
**Next:** Draft `System Prompt NALA v1` yang sudah divalidasi di Langkah 19-20 siap dipindahkan jadi konstanta Python `NALA_SYSTEM_PROMPT` di `Module-04-Setup-Infra-Docker-Compose/PANDUAN-PRAKTIK.md`, Langkah 6 — di sana system prompt yang sama akan dikirim otomatis lewat API tiap ada request masuk ke endpoint `/chat`, bukan diketik manual lagi.
