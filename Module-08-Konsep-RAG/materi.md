...
- Kita bisa menggambarkan alur kerja RAG secara garis besar — dari pertanyaan user sampai jawaban ber-konteks — tanpa perlu tahu detail implementasi (itu Module 9-15)
- Kita paham RAG punya batasan: bisa menyebutkan minimal dua situasi di mana RAG bukan solusi yang tepat
- **Tidak ada kode yang ditulis di module ini** — murni fondasi konsep untuk Module 9-15
...
| Kejujuran saat tidak tahu | Sering tetap menjawab dengan percaya diri | Bisa dirancang untuk mengaku "tidak ditemukan" kalau dokumennya memang tidak ada (Module 12 Bagian 4) |
...
Lima langkah ini, dan di module mana masing-masing dibangun sepanjang Module 6-15:

1. **Pertanyaan user masuk** — sudah ada sejak Module 6 (`/chat`) dan Module 7 (`/chat/stream`), belum berubah bentuknya.
2. **Retrieval** — sistem mencari dokumen mana yang relevan dengan pertanyaan itu, bukan dengan mencocokkan kata kunci secara harfiah, tapi lewat **kemiripan makna** (*semantic search*) — pertanyaan diubah jadi vektor (Module 10, "embedding"), lalu dicari kemiripannya di vector store (Module 11).
3. **Ambil potongan (chunk) paling relevan** — dokumen sungguhan (SOP, kebijakan) biasanya panjang; potongan-potongan kecil ini sudah disiapkan lebih dulu saat proses indexing (Module 9, "chunking") — retrieval di langkah 2 tinggal mengembalikan potongan yang **paling relevan saja**, bukan seluruh dokumen.
4. **Augmented** — potongan yang relevan itu digabungkan ke dalam prompt yang dikirim ke LLM, sebagai "konteks" tambahan sebelum pertanyaan asli (dibangun di Module 12, sekaligus menyatukan retrieval ke `/chat` dan `/chat/stream` — inilah titik RAG benar-benar "hidup" untuk pertama kalinya).
...
Satu proses **tambahan** yang tidak terlihat di alur "tanya-jawab" di atas, tapi wajib terjadi lebih dulu: dokumen harus **masuk** ke knowledge base dan **ter-index** dulu sebelum bisa di-retrieve sama sekali. Itu tugas Module 9-12 (data seed, embedding, vector store, RAG chain pertama) untuk mengisi index awal, lalu Module 13 menambahkan chunking (retrieval jadi lebih presisi) sekaligus form upload web, dan Module 14 (Airflow, otomasi pipeline ingest) menambahkan cara lain data itu bisa terus bertambah — mengisi "rak buku" yang nanti dibuka siswa di langkah retrieval.
...
Tidak ada checklist teknis di module ini (tidak ada kode yang ditulis) — sebagai gantinya, pastikan poin-poin berikut sudah didiskusikan dan dipahami sebelum lanjut ke Module 9:
...
- [ ] Kita paham bahwa Module 9-15 akan membangun, satu per satu, potongan-potongan yang membuat alur RAG di Bagian 4 benar-benar berfungsi

## Kesimpulan

Module ini tidak menulis kode apa pun — tapi menjadi fondasi kenapa Module 9-15 layak dibangun sama sekali. Halusinasi adalah masalah nyata dan struktural pada LLM murni, terutama untuk pertanyaan spesifik perusahaan yang tidak pernah ada di data training model mana pun. RAG mengatasinya bukan dengan melatih ulang model, tapi dengan mengubah LLM dari "siswa ujian closed-book" jadi "siswa ujian open-book" — memberi rujukan yang relevan tepat sebelum menjawab. Alur kerjanya (retrieval → augmented → generation) akan dibangun bertahap, sesederhana mungkin dulu: Module 9 menyiapkan data seed (belum ada chunking), Module 10 mengenalkan cara teks diubah jadi sesuatu yang bisa dicari (embedding), Module 11 menyimpannya di vector store, Module 12 menyatukan semuanya jadi satu alur tanya-jawab yang benar-benar berfungsi — **RAG mulai hidup di sini**, memakai satu vektor per dokumen — sebelum akhirnya Module 13 menambahkan chunking (memecah dokumen jadi potongan yang lebih presisi) sekaligus form upload web, dan Module 14 (Airflow) menambahkan cara lain data terus bertambah di atas fondasi yang sudah bekerja. Tapi RAG bukan solusi ajaib untuk semua masalah — memahami kapan RAG tidak tepat (data transaksional, kebutuhan reasoning, sumber yang belum ter-index) sama pentingnya, dan akan terus relevan sampai Module 20 saat NALA belajar memutuskan sendiri kapan pakai RAG dan kapan pakai tool lain.

## Panduan Praktik

### Prasyarat
- Sudah menyelesaikan **Module 7** (Streaming & Multi-Turn Conversation)

### Langkah 1: Diskusi Konsep RAG

Tidak ada kode atau perintah terminal di langkah ini — Module 8 murni sesi konsep/diskusi, tidak butuh container apa pun menyala. Sebelum lanjut membangun potongan-potongan teknis RAG (Module 9 dan seterusnya), pastikan poin-poin berikut sudah didiskusikan bersama fasilitator: kenapa LLM bisa berhalusinasi, analogi ujian *closed-book* vs *open-book*, definisi RAG, alur kerja RAG secara garis besar (retrieval → augmented → generation), dan kapan RAG **bukan** solusi yang tepat. Lihat bagian-bagian di atas (Bagian 1-5) untuk materi lengkapnya.

Tidak ada Troubleshooting untuk module ini — lanjut ke Module 9 begitu diskusi selesai.
