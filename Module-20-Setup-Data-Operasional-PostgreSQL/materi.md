# Module 20: Setup Data Operasional — PostgreSQL & UI Form

## Tujuan

Menyiapkan sumber data operasional PT Nusantara Finance (pengajuan kredit, klaim asuransi) di PostgreSQL — dengan skema tabel, pemisahan role database sejak awal, dan **halaman web untuk staff menambah data sendiri** — sebagai fondasi yang dipakai Module 22 (SQL Tool) dan seterusnya. Module ini sengaja **tidak** memakai data dummy yang di-generate otomatis lewat skrip seeder besar — datanya sedikit di awal, staff (Anda) yang menambah sisanya lewat form, supaya data yang dipakai untuk latihan terasa seperti hasil kerja nyata, bukan angka yang sudah "disiapkan" begitu saja.

## Definisi

**Data operasional** adalah data transaksional yang berubah setiap hari — pengajuan kredit, klaim asuransi — berbeda secara mendasar dari **knowledge base** (dokumen SOP/kebijakan) yang dipakai RAG sejak Module 8: dokumen SOP jarang berubah dan menjawab pertanyaan "bagaimana prosedurnya", sedangkan data operasional berubah terus dan menjawab pertanyaan "apa status transaksi ini sekarang". Bedanya bukan cuma soal seberapa sering berubah, tapi juga *tempat penyimpanan*: dokumen SOP hidup sebagai vektor di OpenSearch (Module 11), data operasional hidup sebagai baris terstruktur di PostgreSQL — dua sumber jawaban yang akan disatukan lewat agent mulai Module 21.
...
flowchart LR
    ...
    Q["Tool SQL (Module 22)"] -->|"pakai nala_readonly"| T
```
...
- Dua role database terpisah sejak module ini: `nala_readonly` (cuma `SELECT`, dipakai agent/SQL tool di Module 22) dan `nala_writer` (cuma `INSERT`, dipakai form UI) — keduanya **tidak** bisa saling melakukan operasi milik peran yang lain, diverifikasi langsung lewat `psql`
- Halaman baru `/data-operasional` (mengikuti pola `upload.html` Module 14) — form tambah pengajuan kredit & klaim asuransi, plus tabel daftar data yang sudah ada
- Staff sudah menambah beberapa data operasional sungguhan lewat form (bukan lewat SQL manual atau seeder) sebagai bahan uji Module 22 dan seterusnya
...
flowchart LR
    ...
    D --> E["Data operasional siap dipakai<br/>Module 22 (SQL Tool)"]
```

## 2. Dua Role Sejak Awal: Baca vs Tulis

Sebelum menulis kode tool query (baru Module 22), pemisahan role database sudah disiapkan di module ini — supaya begitu Module 22 dibangun, `nala_readonly` sudah ada dan siap dipakai, tidak perlu bolak-balik migrasi:
...
`POSTGRES_PASSWORD` sengaja pakai env var dengan default eksplisit `changeme_dev_only` — cukup untuk laptop training, **bukan** pola aman untuk deployment sungguhan (dibahas lagi soal secret management di Module 24). Image resmi `postgres` otomatis menjalankan file apa pun di `/docker-entrypoint-initdb.d/` **satu kali saat volume database masih kosong** — itu cara `db/seed.sql` (Langkah 2) ter-load otomatis tanpa langkah manual tambahan.
...
-- Role read-only: dipakai agent (tool query_data_operasional, dibangun Module 22) — hanya bisa SELECT
...
role terpisah (read-only vs write-only) — Module 20, Tahap A.
...
CONTEXT:
- Ini tabel BARU, terpisah dari OpenSearch/vector store Module 8-19.
- nala_readonly BELUM dipakai kode apa pun sampai Module 22 (SQL Tool)
...
Dua fungsi terpisah, masing-masing pakai DSN role yang berbeda — `get_connection()` (readonly) dipakai untuk **menampilkan** data di halaman `/data-operasional` dan nanti tool SQL Module 22, `get_write_connection()` (writer) dipakai **hanya** oleh endpoint `POST` form di Langkah 5. Tambahkan juga dua env var ini ke service `api` di `docker-compose.yml`:
...
Mengikuti pola `upload.html` (Module 14) — form di atas, tabel daftar data di bawah, untuk **kedua** tabel (`pengajuan_kredit`, `klaim_asuransi`):
...
pengajuan kredit dan klaim asuransi secara manual — Module 20,
Tahap B.
...
- JANGAN ubah endpoint /chat/stream, /upload, /health yang sudah ada
  (endpoint /chat belum ada sampai Module 21 membuatnya).
```

</details>

**📄 Kode lengkap Module 20** (`docker-compose.yml` bagian `postgres`, `db/seed.sql`, `app/db.py`, `app/templates/data_operasional.html`, potongan relevan `app/main.py` — sudah ditampilkan utuh di Langkah 1-5 di atas).
...
- [ ] Sudah ada beberapa baris data operasional sungguhan (ditambahkan lewat form, bukan SQL manual) sebagai bahan uji Module 22 dan seterusnya

## Kesimpulan

Module ini menyiapkan fondasi data operasional NALA dengan filosofi yang konsisten dengan Module 8 (Knowledge Base): sumber data baru diperkenalkan lewat halaman web yang staff pakai sendiri, bukan data yang tiba-tiba "sudah ada" lewat skrip seeder. Pemisahan role baca/tulis (`nala_readonly`/`nala_writer`) disiapkan sejak awal — supaya begitu Module 21 (LangGraph Agent) dan Module 22 (SQL Tool) dibangun di atasnya, fondasi keamanannya sudah teruji, bukan ditambal belakangan.

Module 21 selanjutnya membangun agent LangGraph (fokus pada tool dokumen SOP dulu, melanjutkan RAG chain Module 8-19) — belum menyentuh data operasional yang baru disiapkan di module ini. Module 22 baru kembali ke data ini, membangun tool SQL yang membaca lewat `nala_readonly` yang sudah ada di sini, lalu mendaftarkannya ke agent yang sudah dibangun Module 21.

## Panduan Praktik

> Catatan penomoran: bagian ini memakai penomoran "Langkah" tersendiri (Langkah 1-5) yang berbeda dari "Langkah 1-5" di dalam Bagian 3 Tahap A/B di atas — keduanya kebetulan bernomor sama tapi berasal dari dua urutan yang terpisah. Penomoran "Langkah" di Panduan Praktik ini juga bersambung lintas-module (Module 21 melanjutkan dari Langkah 6, dst.) sebagai peninggalan dari saat panduan ini masih satu dokumen gabungan Module 20-24.

**Panduan Praktik — Module 20: Setup Data Operasional — PostgreSQL & UI Form**

#### Melanjutkan langsung di folder starter code yang sudah ada

Semua kode dari Module 1-19 hidup di satu folder: `Nala/` (lihat Module 20 bagian Tujuan di atas). Langkah 1 di bawah **tidak** menyalin ke folder baru — rangkaian Module 20-24 di bagian Panduan Praktik berikutnya menambah lapisan data operasional/agent/tool/RBAC **langsung di atas file-file yang sudah ada** di folder ini, module demi module.
...
### Prasyarat
- Sudah menyelesaikan **Module 1-19** (`llama3.2:3b` sudah biasa dipakai, `Nala/` berjalan dengan hybrid search + reranking + evaluasi + Langfuse)
- Docker Desktop sudah dialokasikan resource yang cukup — lihat catatan RAM di bawah

#### Container yang sama dipakai sepanjang Module 20-24
...
Rangkaian Module 20-24 menambahkan **satu service baru**: **PostgreSQL** (data operasional). Dibanding OpenSearch/Airflow, PostgreSQL relatif ringan sendirian — tapi ia berjalan **bersamaan** dengan seluruh stack Module 6-19 yang sudah ada (Ollama, OpenSearch, Airflow, FastAPI) plus proses LangGraph agent yang memanggil Ollama berkali-kali per pertanyaan (multi-turn tool-calling, lihat Module 21-23).

| Setting | Minimal rangkaian Module 20-24 | Direkomendasikan | Alasan |
|---|---|---|---|
| **Memory (RAM)** | 16 GB | 16 GB+ (32 GB+ kalau ingin coba `qwen2.5:7b`, lihat Module 23 Bagian 4.b) | PostgreSQL menambah beban kecil-menengah di atas stack Module 6-19 yang sudah berat; agent tool-calling memanggil Ollama beberapa kali per pertanyaan (bukan sekali seperti sebelumnya), menambah beban CPU sesaat, bukan RAM tambahan besar. |
...
Kalau sudah di 16GB+ sejak Module 6-19, tidak perlu diubah lagi kecuali ingin mencoba `qwen2.5:7b` (Module 23 Bagian 4.b).
...
Folder ini sudah berisi seluruh kode dari Module 1-19 (`app/`, `docker-compose.yml`, `requirements.txt`, dan konfigurasi hybrid search/reranking/Langfuse dari Module 16-19) — rangkaian Module 20-24 **menambah** lapisan data operasional/agent/tool langsung di atas file-file yang sudah ada di sini, bukan menulis ulang dari nol dan bukan menyalin ke folder lain. Verifikasi dulu fondasinya masih utuh sebelum menambah apa pun:
...
✅ **Indikator sukses**: jawaban berbasis dokumen SOP seperti akhir Module 19 (kualitas hybrid search + reranking) — bukti salinan berjalan identik sebelum Module 20 mulai menambah apa pun.

### Langkah 2: Service PostgreSQL, skema, seed minimal, dua role (Module 20 Tahap A)

Ikuti Module 20 Bagian 3 Tahap A Langkah 1-2 (service `postgres` di `docker-compose.yml`, `db/seed.sql` — skema `pengajuan_kredit`/`klaim_asuransi`, seed minimal 2-3 baris, dan **kedua** role `nala_readonly`/`nala_writer` dibuat sejak awal, sekaligus).
...
✅ **Indikator sukses**: `2` dan `1` (sesuai seed minimal — lihat Module 20 Bagian 3 Tahap A Langkah 2).
...
### Langkah 3: Koneksi database — `app/db.py` (Module 20 Tahap B Langkah 3)

Ikuti Module 20 Bagian 3 Tahap B Langkah 3 (`psycopg[binary]` di `requirements.txt`, `app/db.py` dengan dua fungsi `get_connection()`/`get_write_connection()`).
...
### Langkah 4: Halaman `/data-operasional` — form + tabel daftar (Module 20 Tahap B Langkah 4-5)

Ikuti Module 20 Bagian 3 Tahap B Langkah 4-5 (`app/templates/data_operasional.html`, endpoint `GET /data-operasional` + `POST /data-operasional/pengajuan-kredit` + `POST /data-operasional/klaim-asuransi` di `app/main.py`, plus nav link baru di `chat.html`/`upload.html`).
...
### Langkah 5: Isi data operasional lewat form (bahan uji Module 22 dan seterusnya)

Tambahkan data lewat kedua form di `/data-operasional` sampai datanya representatif untuk latihan module-module berikutnya — bukan lewat SQL manual atau seeder. Target yang dipakai di contoh-contoh Module 21-24 selanjutnya (lihat Module 20 Bagian 4 "Hasil Uji Nyata"): total **7 baris** `pengajuan_kredit` (variasi status: `pending`×3, `disetujui`×1, `pencairan`×1, `ditolak`×2 — dua di antaranya sudah ada dari seed Langkah 2: `N-00231` `pending` dan `N-00305` `ditolak` dengan `alasan_penolakan` "Skor kredit di bawah ambang batas minimum", jadi tambahkan **5 baris lagi** lewat form) dan total **4 baris** `klaim_asuransi` (1 sudah ada dari seed — `N-00305`, jenis `kendaraan`, `disetujui` — tambahkan **3 baris lagi**), dengan minimal satu `nasabah_id` yang sengaja muncul di kedua tabel (mis. `N-00305`, yang kreditnya ditolak **dan** punya klaim asuransi — bahan uji skenario gabungan RAG+SQL di Module 23).
...
✅ **Indikator sukses**: `7` dan `4`, dan refresh halaman `/data-operasional` menunjukkan seluruh baris yang baru ditambahkan tetap ada (data tersimpan permanen). Module 20 selesai — lanjut ke bagian Panduan Praktik di materi Module 21.
...
- **`permission denied for table ...` padahal seharusnya diizinkan**: cek role/DSN yang dipakai — form `/data-operasional` (Module 20) harus memakai `nala_writer`, tool SQL (Module 22) harus memakai `nala_readonly`; tertukar salah satunya akan memicu `permission denied` untuk operasi yang seharusnya sah.
