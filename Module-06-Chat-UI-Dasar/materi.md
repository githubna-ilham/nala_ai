...
- Kita punya baseline jawaban "sebelum RAG" untuk dibandingkan setelah Module 12 selesai
- Kita paham `/chat` di module ini adalah **fondasi sementara** — Module 7 akan menggantikannya total dengan `/chat/stream`, bukan menambah endpoint di sampingnya

## 1. Kenapa Mulai dari UI, Bukan Dokumen

Module 1-4 sudah membuktikan bahwa NALA bisa menjawab lewat `curl` — tapi `curl` bukan sesuatu yang akan dipakai staff PT Nusantara Finance sehari-hari. Sebelum Module 6-15 menumpuk kompleksitas baru (chunking, embedding, vector store, Airflow), langkah pertama adalah memastikan NALA bisa diakses lewat **antarmuka web** yang sama, yang nantinya dipakai sepanjang Module 6-28 — tanpa RAG dulu sama sekali.

Ini bukan langkah kosmetik. Ada dua hal konkret yang divalidasi di sini:

1. **Kontrak `/chat` tidak berubah dari Module 1-4 ke Module 6-15.** `ChatRequest{message}` → `ChatResponse{reply}` persis sama — bukti bahwa fondasi Module 1-4 (Docker, Ollama, `NALA_SYSTEM_PROMPT`) memang portable, bukan kebetulan yang cuma jalan sekali. `/chat` sendiri cuma bertahan sampai Module 7 (lihat Bagian 2) — begitu streaming dibutuhkan, endpoint ini digantikan total, bukan dipertahankan di samping yang baru.
2. **Masalah infrastruktur (Docker, network antar-container, model belum ter-pull) terdeteksi lebih awal**, sebelum ditumpuk dengan OpenSearch dan Airflow yang baru dikenalkan di Module 11 dan 14. Kalau chat dasar saja sudah gagal, jauh lebih mudah didiagnosis sekarang daripada setelah 3 service tambahan ikut jalan.

```mermaid
flowchart LR
    subgraph "Module 6 (sekarang)"
...
Tapi kontrak ini **tidak dirancang untuk bertahan selamanya**. Begitu Module 7 butuh streaming (jawaban muncul token demi token) dan riwayat multi-turn (butuh mengirim **array** pesan, bukan satu `message`), kontrak `{"message"} → {"reply"}` sudah tidak cukup — bentuknya sendiri (satu request, satu response JSON utuh) bertentangan dengan cara kerja streaming. Daripada mempertahankan dua endpoint dengan dua kontrak berbeda selamanya (satu untuk kompatibilitas lama, satu untuk fitur baru), Module 7 **mengganti total**: `/chat` dihapus, `/chat/stream` menjadi satu-satunya endpoint chat NALA sejak saat itu — konsisten dengan keputusan "NALA seluruhnya berbasis streaming mulai Module 7" (lihat Module 7 Bagian 2 untuk alasan lengkapnya).

`chat.html` sendiri **akan berubah lebih awal dari yang mungkin terlihat wajar** — sudah di Module 7 (bukan menunggu sampai Module 12), karena streaming dan riwayat multi-turn butuh cara berbeda mengirim dan menampilkan pesan. Anggap Module 6 sebagai **jembatan sekali pakai**: cukup untuk membuktikan UI ↔ FastAPI ↔ Ollama tersambung, sebelum bentuk akhirnya (streaming) dibangun di atasnya.
...
Belum ada alasan untuk mengubah isi file yang sudah ada — perubahan baru muncul di Tahap C, begitu kita butuh sesuatu yang Module 1-4 tidak punya (merender HTML).
...
GOAL:
- Verifikasi 6 file berikut ada dan isinya belum berubah dari Module 1-4:
  - docker-compose.yml
  - Dockerfile
  - requirements.txt
...
GUARDRAIL:
- JANGAN ubah isi 6 file di atas — biarkan persis seperti Module 1-4.
...
```
Buat halaman chat NALA (Module 6, Tahap B) — chat.html dan style.css,
...
Pola ini akan berulang di module-module berikutnya: `python-multipart` ditambahkan tepat saat Module 13 butuh upload file, dependency OpenSearch/Airflow ditambahkan tepat saat Module 11 dan 14 membutuhkannya — bukan diborong semua di awal.
...
**📄 Kode lengkap Tahap C** (versi final Module 6):
...
Untuk menjaga fokus, Module 6 sengaja **tidak** menyentuh:
- Streaming atau riwayat multi-turn (Module 7 — endpoint `/chat/stream` **menggantikan** `/chat`, dibangun di atas fondasi module ini)
- Konsep RAG (Module 8), data seed (Module 9, belum ada chunking)
- Embedding (Module 10), vector search (Module 11)
- Retrieval yang benar-benar dipakai `/chat/stream` (Module 12, versi tanpa chunking)
- Chunking (Module 13) dan upload dokumen (Module 14, halaman `/upload`)
- Airflow (Module 15)

Kalau dicoba di titik ini, jawaban NALA akan terasa "biasa saja" — sama seperti Module 1-4 — dan itu **memang tujuannya**: memberi kita garis dasar (baseline) yang jelas untuk dibandingkan setelah RAG mulai aktif di Module 12. Perbedaan sebelum-dan-sesudah ini adalah salah satu momen belajar paling nyata di Module 6-15.

## 5. Checkpoint Praktik

Langkah eksekusi lengkap (menjalankan `docker compose up`, membuka `http://localhost:8000`, dan mencoba chat) ada di **bagian Panduan Praktik di bawah, Langkah 1-4**. Yang perlu dipastikan sebelum lanjut ke Module 7:
...
Begitu ketiga hal ini terverifikasi, lanjut ke Module 7 — yang membuat balasan NALA terasa hidup lewat streaming dan mengingat konteks percakapan sebelumnya (multi-turn), sebelum Module 6-15 menumpuk kompleksitas dokumen/RAG.

## Panduan Praktik

### Satu folder kode untuk Module 6-28

- **`Nala/`** — satu-satunya folder kerja Anda, dipakai sejak Module 1-4 dan terus sama sepanjang Module 6-28. Panduan Module 6-15 membangunnya **bertahap, module demi module, langsung di tempat**: setiap module menambah/mengubah file di folder ini, tidak ada penyalinan ke folder lain. Semua perintah di panduan ini dijalankan di folder ini.
...
`Nala/` adalah folder yang **sama persis** dengan yang Anda pakai di Module 1-4 — `ollama_client.py`, `system_prompt.py`, `main.py`, `docker-compose.yml`, dan seterusnya sudah ada di sana, dan fitur-fitur baru Module 6-15 (streaming, multi-turn, RAG chain, chunking, embedding, vector store, upload, Airflow) ditambahkan langsung di atasnya, di tempat yang sama.
...
Module 6-15 menambahkan **dua service baru** di atas stack Module 1-4: **OpenSearch** (vector store, mulai Module 11) dan **Airflow** (orchestrator, mode `standalone`, mulai Module 15). Kedua service ini jauh lebih berat dibanding FastAPI/Ollama saja — OpenSearch adalah JVM yang butuh heap tersendiri, dan Airflow standalone menjalankan webserver + scheduler + database sekaligus dalam satu container.

Kalau di Module 1-4 Anda mengalokasikan Docker Desktop di batas minimal (8–12GB), **naikkan ke 16GB+** untuk Module 6-15. Ikuti langkah yang sama seperti **Langkah 0 di `Module-04-Setup-Infra-Docker-Compose/materi.md`, bagian Panduan Praktik** (Docker Desktop → ⚙️ Settings → tab Resources):

| Setting | Minimal Module 6-15 | Direkomendasikan | Alasan |
|---|---|---|---|
...
Catatan: panduan Module 6-15 menyalakan service **secara bertahap**, mengikuti urutan module: `ollama`+`api` dulu di Langkah 2 di bawah (Module 6-9 cuma butuh ini — Module 8 malah tidak butuh service apa pun, murni diskusi), `opensearch` menyusul di Module 11, `airflow` terakhir di Module 15 — jadi beban RAM di Module 6-10 jauh lebih ringan dari 16GB. Alokasi 16GB+ tetap perlu disiapkan sebelum Module 15, begitu keempat service jalan bersamaan.
...
Stack Module 6-15 total punya 4 service (`ollama`, `opensearch`, `airflow`, `api`), tapi Module 6-9 (chat UI, streaming/multi-turn, konsep RAG, data seed — Module 8 murni diskusi konsep, tidak butuh service sama sekali) cuma butuh **dua yang pertama** — `opensearch` baru dipakai mulai Module 11, `airflow` baru dipakai mulai Module 15. Supaya urutan belajar juga terasa di infrastrukturnya — bertahap, bukan langsung semua nyala sekaligus — nyalakan `ollama` dan `api` saja dulu, skip dua yang lain:
...
Ini aman meski `api` "seharusnya" nantinya butuh OpenSearch — belum sekarang. Di titik ini (`Nala/` baru sampai Module 6), `/chat` belum menyentuh OpenSearch sama sekali. Nanti begitu Module 12 (`/chat/stream` jadi RAG-aware) dan Module 14 (`/upload`) selesai dibangun, kedua endpoint itu akan membungkus setiap panggilan ke OpenSearch dalam `try/except httpx.HTTPError` — kalau `opensearch` tidak jalan, koneksi gagal secara terkontrol dan otomatis fallback (mode tanpa-konteks untuk chat, "tersimpan tapi belum ter-index" untuk upload), bukan crash. Lihat Module 12 Bagian 2 Langkah 3 dan Module 14 Bagian 2 Tahap B untuk penjelasan desainnya begitu Anda sampai di situ.
...
NALA akan tetap membalas — koneksi UI → FastAPI → Ollama sudah bekerja — tapi jawabannya generik karena belum ada dokumen internal untuk dirujuk sama sekali (belum ada RAG di titik ini). Tujuannya memverifikasi dulu bahwa fondasi chat sudah jalan, sebelum lapisan streaming (Module 7) dan RAG (Module 9-15) ditambahkan di atasnya.

⚠️ **`/chat` di langkah ini bersifat sementara** — Module 7 Langkah 1 akan **menghapus total** endpoint ini dan menggantikannya dengan `/chat/stream`. Sengaja dibangun dulu di sini supaya fondasi (Docker, network, model) terverifikasi dengan kontrak paling sederhana, sebelum kompleksitas streaming ditambahkan.
