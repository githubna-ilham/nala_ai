# Module 19: Observability & Tracing dengan Langfuse

## Tujuan

Menambahkan Langfuse self-hosted untuk mencatat setiap request `/chat/stream` (satu-satunya endpoint chat NALA sejak Module 7) sebagai satu trace terstruktur (retrieval, rerank, generation), supaya jawaban NALA yang terlihat buruk bisa ditelusuri persis di tahap mana masalahnya muncul — melengkapi evaluasi batch Module 18 dengan visibilitas per-request.
...
- Service `langfuse-db` (Postgres khusus trace, terpisah dari Postgres data operasional Module 20) dan `langfuse` (v2, self-hosted) berjalan lewat Docker Compose, bisa diakses di `http://localhost:3000`
...
- Trade-off dicatat jujur: overhead latensi tambahan, enam container berjalan bersamaan (beban RAM terbesar sepanjang Module 1-19), dan data trace berisi potongan dokumen internal (aman karena self-hosted, jadi perhatian kalau dipindah ke cloud)

## 1. Kenapa "Baca Log Container" Tidak Lagi Cukup

Di Module 1-15, kalau `/chat/stream` menjawab aneh, menelusurinya masih realistis lewat `docker compose logs api` — alurnya pendek: terima pesan, panggil Ollama, kembalikan jawaban. Setelah Module 16-18, alur `/chat/stream` sudah jauh lebih panjang dan setiap tahap bisa jadi sumber masalah:
...
**Observability** menyelesaikan ini dengan mencatat setiap tahap sebagai satu **trace** terstruktur per request — bukan menggantikan Module 18 (evaluasi batch, mengukur kualitas rata-rata di banyak pertanyaan), tapi melengkapinya untuk kasus "satu jawaban tertentu ini kenapa salah, tepatnya di mana".
...
## 3. Postgres Langfuse ≠ Postgres Module 20

Penting untuk tidak tertukar: Postgres yang ditambahkan module ini (`langfuse-db`) **khusus untuk menyimpan data trace Langfuse** — bukan Postgres yang akan diperkenalkan Module 20 untuk data operasional (pengajuan kredit, klaim, dst, dipakai agent tool query SQL). Keduanya berdiri sendiri, database terpisah, tujuan berbeda. Module 20 akan menambah service Postgres-nya sendiri (`postgres` atau nama serupa) — jangan menyatukan keduanya walau sama-sama "Postgres".
...
- langfuse-db TERPISAH dari Postgres data operasional yang akan
  ditambahkan Module 20 — jangan digabung.
...
- **`traced_chat_stream()`** adalah generator **baru** yang membungkus `ollama_client.chat_stream()` yang sudah ada sejak Module 7 — bukan menggantikannya. Ia meneruskan (`yield token`) setiap token persis seperti aslinya, sambil diam-diam mengumpulkannya ke `accumulated`.
...
✅ **Indikator sukses**: streaming tetap berjalan token demi token seperti Module 7 (tidak ada perubahan perilaku dari sisi user), dan trace baru untuk `chat_stream` muncul di `http://localhost:3000` (halaman **Traces** project Anda) **setelah** curl selesai menerima seluruh stream (bukan langsung saat request dikirim) — bukti `flush()` di dalam generator benar-benar menunggu token terakhir, bukan terpanggil prematur. Klik trace tersebut untuk melihat span `hybrid_search`, `rerank`, dan generation `llm_generate_stream` tersusun bersarang dengan waktu eksekusi masing-masing.
...
- Alur retrieval-rerank-generate di chat_stream() sudah lengkap dari
  Module 16-17 — module ini HANYA menambah instrumentasi observability
...
**📄 Kode lengkap** (bagian relevan `app/main.py` setelah Module 19 — lihat Bagian 4 Tahap B untuk fungsi `traced_chat_stream()` dan `chat_stream()` secara utuh; setup `langfuse_client` di Bagian 4 Tahap B Langkah 3).
...
Tiap trace juga punya field `scores` (terlihat kosong `[]` kalau dicek lewat API `GET /api/public/traces`, atau tab **Scores** di UI) — ini **normal** untuk module ini, bukan bug. "Scores" di Langfuse adalah fitur terpisah untuk melekatkan penilaian kualitas ke sebuah trace (dari manusia lewat UI, atau dari kode). Module 19 ini cuma fokus mencatat **apa yang terjadi** (trace/span/generation), belum menilai **seberapa bagus** hasilnya.

Koneksi yang belum dimanfaatkan di sini: `judge_answer()` dari Module 18 (`app/llm_judge.py`) menghasilkan skor `faithfulness`/`relevance` — secara konsep persis jenis data yang cocok dikirim lewat `trace.score(name=..., value=..., comment=...)`, supaya skor itu langsung terlihat melekat di tiap trace UI, bukan cuma di output terpisah `run_evaluation.py`. Ini di luar cakupan Module 19 yang sudah ditentukan (fokus instrumentasi dasar dulu), tapi dicatat di sini sebagai perluasan alami yang masuk akal untuk pengembangan lebih lanjut.
...
- Riwayat trace historis berguna sebagai bahan tambahan untuk memperluas test set Module 18 — pertanyaan nyata dari staff yang jawabannya buruk bisa dijadikan entri baru di `QA_TESTSET`.
...
- **Dua service tambahan** (`langfuse`, `langfuse-db`) menambah beban RAM di atas stack yang sudah berat sejak Module 17 (reranker). Ini kemungkinan besar titik dengan jumlah service **terbanyak** sepanjang Module 1-19: `ollama`, `opensearch`, `airflow`, `api` (dengan reranker), `langfuse`, `langfuse-db` — enam container berjalan bersamaan. Kalau laptop mulai terasa berat, pertimbangkan mematikan sementara `airflow` (tidak dipakai lagi setelah ingest awal selesai, lihat pola staged startup di Module 15 materi.md, bagian Panduan Praktik) selama sesi eksplorasi Langfuse berlangsung.
...
Langkah eksekusi lengkap ada di bagian **Panduan Praktik** di bawah, Langkah 1-6. Yang perlu dipastikan sebelum Module 16-19 dianggap selesai:
...
Module ini menutup rangkaian Module 16-19 dengan lapisan yang membungkus **seluruh** yang sudah dibangun — hybrid search (Module 16), reranking (Module 17), dan evaluasi batch (Module 18) — dengan visibilitas per-request. Sebelumnya, kalau ada satu jawaban NALA yang terlihat buruk, jalan satu-satunya adalah menduga-duga atau membaca log mentah; sekarang, tiap trace menunjukkan persis apa yang terjadi di tiap tahap (kandidat yang ditemukan, urutan setelah rerank, prompt lengkap yang dikirim ke LLM, jawaban akhir), lengkap dengan waktu eksekusinya.

**Module 16-19 secara keseluruhan** mengangkat NALA dari sistem RAG dasar (Module 6-15, retrieval vector murni, tidak terukur) menjadi sistem yang lebih akurat retrievalnya (hybrid search + reranking), terukur kualitasnya (framework evaluasi), dan bisa didiagnosis per-request (observability). Yang **belum** disentuh sejauh ini: NALA masih hanya bisa menjawab dari dokumen SOP — belum bisa menjawab pertanyaan yang jawabannya ada di data operasional terstruktur (status pengajuan kredit tertentu, riwayat klaim seorang nasabah, dst). Module 20-24 menambah agentic tools: NALA belajar memilih kapan menjawab dari RAG dokumen (yang baru saja disempurnakan sepanjang Module 16-19 ini) dan kapan menjalankan query SQL langsung ke database operasional — dengan Langfuse yang sudah terpasang di module ini siap merekam trace kedua jalur itu sekaligus.
...
### Prasyarat
- Sudah menyelesaikan **Module 18** — `Nala/` sudah punya framework evaluasi bekerja
- Docker Desktop dinaikkan lagi alokasi RAM-nya untuk menampung dua service baru:

| Setting | Minimal | Direkomendasikan | Alasan |
|---|---|---|---|
| **Memory (RAM)** | 16 GB | 20 GB+ jika tersedia | Semua service sebelumnya (Ollama, OpenSearch, Airflow, api dengan reranker) + Langfuse & Postgres-nya (`langfuse-db`, terpisah dari Postgres data operasional yang baru akan muncul di Module 20) berjalan bersamaan di titik puncak. |
...
Ikuti Module 19 Bagian 4 Tahap A Langkah 1: tambah service `langfuse-db` dan `langfuse` di `docker-compose.yml`.
...
Ikuti Module 19 Bagian 4 Langkah 2:
...
Ikuti Module 19 Bagian 4 Tahap B Langkah 3-4 — perhatikan baik-baik penjelasan di sana tentang kenapa `generation` (pemanggilan LLM) harus ditutup **di dalam generator**, bukan di badan fungsi endpoint, sementara span `hybrid_search`/`rerank` tetap bisa dibuka/ditutup langsung di badan `chat_stream()`.
...
Buka trace yang baru dibuat di Langkah 3, klik untuk membuka detail. Verifikasi span `hybrid_search`, `rerank`, dan generation `llm_generate_stream` tersusun bersarang dengan `input`/`output` yang masuk akal, dan perhatikan durasi tiap span. Ikuti alur diagnosis lengkap di Module 19 Bagian 5 sebagai latihan.
...
Airflow tidak dipakai lagi setelah ingest dokumen awal selesai — mematikannya sementara selama eksplorasi Module 18-19 membebaskan RAM untuk reranker dan Langfuse tanpa mengganggu `/chat/stream`, yang tidak bergantung pada Airflow sama sekali. Nyalakan lagi (`docker compose up -d airflow`) kapan pun dibutuhkan lagi (misalnya untuk ingest dokumen baru).
