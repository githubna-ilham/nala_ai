...
Menambahkan **cara lain** mengisi index — lewat DAG Airflow yang bisa di-trigger manual dari UI/CLI, memanggil `ingest_documents()` (jamak: scan **seluruh folder**, dibuat di Module 14, di-upgrade untuk chunking di Module 15). Beda dari `/upload` (Module 16, yang memanggil `ingest_document()` tunggal untuk **satu** file reaktif lewat web), Airflow cocok untuk skenario batch/operasional yang lebih besar: banyak dokumen sekaligus (termasuk yang ditaruh langsung ke folder tanpa lewat form upload), audit trail, dan siap upgrade ke terjadwal.
...
Wajar muncul pertanyaan ini: `/upload` (Module 16) sudah otomatis meng-index dokumen begitu diupload — kenapa masih butuh Airflow? Jawabannya ada di **granularitas** fungsi yang dipanggil masing-masing: `/upload` memanggil `ingest_document()` (tunggal — cuma **satu** file yang baru diupload), sedangkan Airflow memanggil `ingest_documents()` (jamak — scan **seluruh** folder `knowledge-base/`). Keduanya sama-sama "fungsi inti" dari Module 14, tapi cakupannya sengaja beda — dan justru beda cakupan inilah yang membuat keduanya saling melengkapi, bukan bersaing. Untuk skenario **satu file diupload lewat form web**, Airflow memang tidak wajib — `/upload` saja sudah cukup. Nilai Airflow muncul di skenario **lain** yang tidak bisa dicakup `/upload`:

- **Reprocessing massal**: kalau banyak dokumen ditaruh langsung ke folder `knowledge-base/` (bukan lewat form upload, misal staff IT copy file langsung ke server), tidak ada yang otomatis men-trigger indexing — `/upload` tidak pernah dipanggil sama sekali dalam skenario ini, jadi `ingest_document()` (tunggal) tidak relevan di sini. Airflow (`ingest_documents()`, jamak) jadi cara menjalankan ulang seluruh proses kapan pun dibutuhkan, tanpa login server dan jalankan script manual (persis seperti yang Anda lakukan manual di Module 14 Bagian 1 dan Module 15 Bagian 8).
...
**Kesimpulan praktis**: `/upload` (Module 16) = jalur cepat untuk kasus umum (1 dokumen, upload manual lewat UI, `ingest_document()` tunggal). Airflow = jalur untuk kasus operasional yang lebih besar (batch, terjadwal, butuh retry/audit, `ingest_documents()` jamak) — keduanya beda pemicu **dan** beda cakupan (satu file vs seluruh folder), tapi sama-sama memanggil fungsi yang sudah dibangun di Module 14 (lalu di-upgrade chunking Module 15) tanpa logika baru.
...
Sekarang trigger DAG `ingest_documents` — memanggil `ingest_documents()` (jamak), fungsi yang sama persis dengan yang dipanggil manual di Module 14-15 (scan seluruh folder), bukan `ingest_document()` (tunggal) yang dipakai form upload di Module 16. Buka `http://localhost:8080`, login dengan `admin` dan password yang tercetak di log container `airflow` saat startup (atau cari lagi dengan cara di Troubleshooting), cari DAG `ingest_documents`, aktifkan toggle-nya (kalau masih off), lalu klik tombol **Trigger DAG** (dua opsi trigger — UI atau CLI — dibahas lengkap di Bagian 6).
