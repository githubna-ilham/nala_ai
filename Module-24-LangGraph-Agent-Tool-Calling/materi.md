...
- Logika retrieval RAG Module 9-22 sudah di-refactor jadi tool berdiri sendiri (`rag_search()` + `RAG_TOOL_SCHEMA`) yang mengembalikan teks konteks, bukan langsung memanggil LLM — dibuktikan lewat uji nyata (Bagian 7a) memakai `search_hybrid()` + reranker Module 18-20, bukan `search()` polos, supaya tidak regresi kualitas
- Graph LangGraph (`call_model` ↔ `call_tool`, dengan edge kondisional) sudah dibangun dan disambungkan ke endpoint `POST /chat` yang **baru dibuat** di module ini — logikanya adalah refactor dari retrieval yang sebelumnya hardcoded di `/chat/stream` (Module 9-22), bukan mengganti isi `/chat` yang sudah ada
...
Yang membuat ini "agent" (bukan cuma percabangan `if/else` biasa) adalah **keputusan tool mana yang dipanggil, dan kapan berhenti, diambil oleh LLM itu sendiri** — bukan oleh aturan `if "kredit" in pertanyaan` yang ditulis manual oleh developer. Ini penting dipahami sebelum Module 27: kualitas keputusan ini bergantung pada kemampuan model, bukan pada logika kode — itu sebabnya routing bisa salah (lihat Module 27 Bagian 4), dan itu bukan bug di kode.
...
✅ **Indikator sukses**: tidak ada error, output berupa potongan teks dari `sop-pengajuan-kredit.md` (asumsi index OpenSearch sudah terisi dari Module 7-22) — bukti `rag_search()` bekerja identik dengan retrieval yang sebelumnya ada di `/chat/stream`, cuma sekarang bisa dipanggil terpisah. Kalau hasilnya kosong, index OpenSearch mungkin belum terisi — jalankan ulang ingest lewat Airflow (Module 17).
...
- embed_text ada di app/embeddings.py, VectorStore ada di
  ...
  diperbarui Module 18-19 untuk hybrid search — jangan ubah keduanya).
...
✅ **Indikator sukses**: tidak ada error `500`, jawaban tetap berbasis dokumen SOP seperti sejak Module 9-22 (kualitasnya seharusnya setara, karena `rag_search()` adalah refactor, bukan perubahan logika retrieval). Cek juga log container `api` — kalau memungkinkan, tambahkan `print()` sementara di `call_model`/`call_tool` untuk melihat bahwa graph benar-benar melewati kedua node itu (dihapus lagi setelah verifikasi, atau ganti dengan `logging` semestinya — audit logging asli baru dibangun di Module 27).
...
sudah dibangun di Langkah 5 (Module 24, Tahap C, Langkah 6) — logika
...
**📄 Kode lengkap Module 24** (`app/agent.py`, `app/tools/rag_tool.py`, potongan relevan `app/main.py` — sudah ditampilkan utuh di Langkah 3, 5, 6 di atas; tidak ada file lain yang berubah).

## 5. Apa yang TIDAK Ada di Module Ini

- Tool kedua (SQL ke data operasional) — Module 25.
- Routing eksplisit antara dua tool, termasuk kasus gagal — Module 27 (module ini baru punya **satu** tool, jadi "routing" satu-satunya keputusan yang ada adalah "pakai tool atau tidak", belum "pakai tool yang mana").
...
- [ ] `/chat` (endpoint baru) menjawab pertanyaan seputar SOP dengan kualitas setara `/chat/stream` Module 9-22 (logika retrieval-nya refactor, bukan regresi)
...
Kode di Langkah 3 (Bagian 4 Tahap B) menunjukkan `rag_search()` memanggil `vector_store.search()` — vector search murni. Tapi Module 18-20 sudah membangun `search_hybrid()` (Module 18-19) dan `Reranker` (Module 20) yang jauh lebih akurat (dibuktikan dengan angka nyata: Hit Rate@3 naik dari 80% ke 100% setelah reranking, lihat Module 21 Bagian 9). Kalau `rag_search()` dibuat memakai `search()` polos, itu **regresi** — tool RAG di agent ini akan lebih buruk dari `/chat/stream` versi Module 18-22 yang menjadi acuan kualitasnya.

Implementasi final `rag_search()` diperbaiki supaya konsisten dengan Module 18-20:
...
Setelah `/chat` (endpoint baru) dibangun di atas agent, trace `chat_agent` yang muncul di Langfuse ternyata **`latency: 0`, `observations: 0`** — dibandingkan trace `chat_stream` (masih pipeline lama Module 18-22) yang tetap `latency: ~14s`, `3 observations`. Ini ditemukan lewat inspeksi trace UI langsung, bukan dari membaca kode.
