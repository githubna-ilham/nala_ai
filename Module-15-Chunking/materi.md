...
Menambahkan **chunking** — memecah dokumen jadi potongan-potongan kecil sebelum di-embed — dan meng-upgrade `ingest_document()` (Module 14) dari "satu dokumen = satu vektor" jadi "satu chunk = satu vektor". `ingest_documents()` (jamak) ikut ter-upgrade otomatis tanpa disentuh, karena cuma memanggil `ingest_document()` di dalam loop-nya. Module ini murni tentang mesin RAG-nya sendiri (`app/ingest.py`, `app/main.py`) — belum menyentuh cara staff menambah dokumen baru, itu baru Module 16 (Upload).
...
Sekarang saatnya mengatasi itu. Alasan yang sama juga berlaku ke depan untuk Module 16 (Upload): staff bisa saja mengupload dokumen yang jauh lebih panjang dari dua SOP contoh (Module 10 Bagian 2), atau PDF berhalaman banyak yang mendekati/melewati batas context window model embedding (Module 11 Bagian 2). Membangun chunking di module tersendiri, **sebelum** Upload dibangun, berarti begitu Module 16 datang, ia tinggal memanggil `ingest_document()` yang sudah level-chunk untuk **satu** file yang baru diupload — tidak perlu mikirkan chunking sebagai bagian dari alur upload itu sendiri.
...
`app/ingest.py` sekarang berisi `extract_text()` (Module 10), `ingest_document()`, dan `ingest_documents()` (keduanya Module 14). Dua Tahap: **Tahap A** — `chunk_text()`, pure function tanpa I/O; **Tahap B** — `chunk_markdown()`, versi structure-aware khusus `.md`. Keduanya belum butuh Ollama atau OpenSearch, jadi tidak ada perubahan `docker-compose.yml` di tahap ini — upgrade `ingest_document()` untuk memakainya baru terjadi di Bagian 8.
...
CONTEXT:
- extract_text(), ingest_document(), dan ingest_documents() (versi
  tanpa chunking, dari Module 14) sudah ada di file yang sama —
  jangan diubah di langkah ini.
- Fungsi ini akan dipakai untuk meng-upgrade ingest_document() di
...
- JANGAN ubah extract_text(), ingest_document(), atau
  ingest_documents() yang sudah ada.
...
- Dispatch-nya (ingest_document() memilih chunk_markdown() untuk
  .md, chunk_text() untuk selain itu) baru ditulis di Bagian 8.

GUARDRAIL:
- JANGAN ubah ingest_document() atau ingest_documents() yang sudah
  ada dari Module 14 — itu diubah di Bagian 8, bukan di sini.
...
## 8. Upgrade `ingest_document()`: dari Level-Dokumen ke Level-Chunk

Sekarang `chunk_text()` dan `chunk_markdown()` sudah ada (Bagian 7) — saatnya mengubah `ingest_document()` (Module 14 Bagian 1) supaya memecah dokumen jadi beberapa chunk **sebelum** di-embed, bukan meng-embed dokumen utuh. Yang diubah **sengaja `ingest_document()` (tunggal)**, bukan `ingest_documents()` (jamak) — chunking adalah soal bagaimana **satu** dokumen diproses, dan itu logikanya sudah dipusatkan di `ingest_document()` sejak Module 14. `ingest_documents()` tetap jadi loop tipis di atasnya, tidak perlu disentuh sama sekali — begitu `ingest_document()` mengembalikan jumlah chunk (bukan lagi `None`), `ingest_documents()` tinggal menjumlahkannya.

**Langkah 3 — Ubah `ingest_document()` di `app/ingest.py`**

```python
# app/ingest.py — ganti isi ingest_document() yang sudah ada, dan return type-nya
...
```python
# app/ingest.py — ingest_documents() TIDAK diubah isinya, cuma otomatis ikut ter-upgrade
...
Yang berubah dibanding Module 14 Bagian 1: (1) setelah `extract_text()`, konten dipecah dulu — `chunk_markdown()` untuk `.md`, `chunk_text()` untuk `.txt`/PDF; (2) loop baru `for i, chunk in enumerate(chunks)` di dalam `ingest_document()` — tiap **chunk** (bukan seluruh dokumen sekaligus) di-embed dan di-index terpisah; (3) `doc_id` berubah dari `filename` jadi `f"{filename}-{i}"` — supaya tiap chunk dari dokumen yang sama tetap dapat ID unik, deterministik (idempotent, sama seperti dijelaskan Module 14 Bagian 1); (4) return value `ingest_document()` berubah dari `None` (Module 14) jadi `int` — jumlah chunk dari dokumen itu, dipakai `ingest_documents()` untuk menjumlahkan total.

`ingest_documents()` di atas terlihat sama persis seperti versi Module 14 — **memang sengaja tidak diubah baris kodenya sama sekali**. Ini bukti langsung kenapa Module 14 memisahkan dua fungsi ini: begitu `ingest_document()` (tunggal) di-upgrade untuk chunking, `ingest_documents()` (jamak) otomatis ikut memproses secara chunk **tanpa perlu disentuh** — cukup mewarisi perilaku baru lewat pemanggilan `ingest_document()` di dalam loop-nya.
...
- Di Nala/app/ingest.py: ganti ISI fungsi
  ...
  Ganti return-nya dari None jadi return len(chunks) — ubah juga
  signature fungsi dari `-> None` jadi `-> int`. JANGAN ubah bagian
  ensure_index()/VectorStore setup di awal fungsi.

CONTEXT:
- chunk_text() dan chunk_markdown() sudah ada dari Bagian 7 (Tahap A, B).
- ingest_document() versi lama (Module 14, tanpa chunking, return None)
  sudah ada dan sedang di-upgrade di sini — bukan dibuat dari nol.
- ingest_documents() (jamak) memanggil ingest_document() di dalam
  loop-nya dan menjumlahkan return value-nya — TIDAK perlu diubah
  sama sekali, otomatis ikut menghitung total chunk yang benar begitu
  ingest_document() di-upgrade.

GUARDRAIL:
- JANGAN ubah extract_text(), chunk_text(), atau chunk_markdown().
- JANGAN ubah ingest_documents() — biarkan apa adanya dari Module 14.
...
- ingest_document() sudah di-upgrade untuk memakai chunking di
  Langkah 3 (ingest_documents() ikut ter-upgrade otomatis) — index
  sekarang berisi puluhan chunk, bukan segelintir dokumen utuh, jadi
  top_k=2 terlalu kecil.
