...
Workflow seperti ini bisa dijalankan dengan dua cara: **trigger manual**, saat seseorang menekan tombol atau menjalankan perintah (dipakai sepanjang Module 6-15, Bagian 6), atau **terjadwal (scheduled)**, saat workflow jalan otomatis mengikuti jadwal cron tanpa campur tangan manusia sama sekali (Bagian 6.3, opsional untuk training ini).
...
## 2. Kenapa NALA Butuh Airflow — Padahal Upload (Module 14) Sudah Otomatis Meng-index

Wajar muncul pertanyaan ini: `/upload` (Module 14) dan Airflow sama-sama memanggil `ingest_documents()` yang sama persis (dibuat Module 12, di-upgrade chunking Module 13). Untuk skenario **satu file diupload lewat form web**, Airflow memang tidak wajib — `/upload` saja sudah cukup. Nilai Airflow muncul di skenario **lain** yang tidak dicakup `/upload`:

- **Reprocessing massal**: kalau banyak dokumen ditaruh langsung ke folder `knowledge-base/` (bukan lewat form upload, misal staff IT copy file langsung ke server), tidak ada yang otomatis men-trigger indexing — Airflow jadi cara menjalankan ulang seluruh proses kapan pun dibutuhkan, tanpa login server dan jalankan script manual (persis seperti yang Anda lakukan manual di Module 12 Bagian 1 dan Module 13 Bagian 8).
...
**Kesimpulan praktis**: `/upload` (Module 14) = jalur cepat untuk kasus umum (1 dokumen, upload manual lewat UI). Airflow = jalur untuk kasus operasional yang lebih besar (batch, terjadwal, butuh retry/audit) — keduanya cuma beda pemicu dan jaminan yang didapat, memanggil fungsi inti yang **sama persis** (`ingest_documents()`, dibuat Module 12, di-upgrade Module 13).
...
**Constraint:** Prasyarat training adalah minimum 16GB RAM **untuk semua services Module 6-28** (Ollama, FastAPI, OpenSearch, PostgreSQL operational data, Langfuse, LangGraph, **plus Airflow**). Dengan prasyarat ini, tidak cukup ruang untuk Airflow production setup.
...
**Untuk Module 15 training ini, DAG disederhanakan jadi satu task saja** (fokus belajar konsep DAG dan scheduling, bukan kompleksitas multi-task dependency):

```mermaid
flowchart TD
    subgraph dag["ingest_documents_dag (Simplified untuk Module 15 Training)"]
...
| `run_ingest()` | Fungsi wrapper lokal yang memanggil `ingest_documents()` dari `app/ingest.py` (dibuat Module 12, di-upgrade chunking Module 13) dengan argumen `folder_path="/opt/airflow/knowledge-base"` — wrapper diperlukan karena Airflow `python_callable` tidak bisa langsung menyuplai argumen |
| `with DAG(...) as dag:` | Membuat instance DAG, `dag_id="ingest_documents"` |
| `schedule=None` | DAG tidak dijadwalkan otomatis — **trigger manual saja** untuk Module 15 |
...
Untuk Module 15 training, DAG `ingest_documents` dijadwalkan **manual trigger only** (`schedule=None`).
...
| Untuk Module 15 | ✅ Ini yang dipakai | ❌ Extension opsional |

**Extension (opsional, di luar cakupan wajib Module 15)** — upgrade ke scheduled:
...
Cuma **satu Tahap** — `ingest_documents()` (Module 12, di-upgrade Module 13) dan upload (Module 14) sudah selesai duluan, module ini murni menambahkan **service Airflow + DAG**, tidak ada perubahan `app/ingest.py` atau `app/main.py` sama sekali.
...
CONTEXT:
- app/ingest.py sudah punya ingest_documents() dari Module 12 (versi
  chunking dari Module 13) — DAG ini cuma memanggilnya, tidak menulis
  ulang logikanya.
...
Yang perlu dipastikan sebelum Module 15 dianggap selesai:

- [ ] DAG `ingest_documents` bisa di-trigger dari Airflow UI dan berstatus `success`
- [ ] `curl http://localhost:9200/nala-docs/_count` menunjukkan jumlah dokumen yang sesuai
- [ ] Kita paham kapan pakai upload (Module 14) vs Airflow (module ini) — bukan dua sistem yang bersaing
- [ ] Dokumen yang di-ingest lewat Airflow langsung bisa ditanyakan ke `/chat/stream` tanpa perubahan kode apa pun di Module 12
- [ ] `/chat/stream` (Module 12) masih menjawab dari dokumen seperti sebelumnya

Begitu kelima hal ini terverifikasi, Module 15 selesai — NALA sudah punya RAG chain lengkap dari dokumen mentah sampai jawaban ber-konteks, dengan **tiga** cara data bisa masuk (seed manual, upload web, Airflow), semuanya bermuara ke fungsi `ingest_documents()` yang sama.

## Panduan Praktik

### Prasyarat
- Sudah menyelesaikan **Module 14** (Upload Dokumen)
- RAM Docker Desktop di 16GB+ (lihat Module 6, bagian Panduan Praktik — catatan RAM) — di langkah ini keempat service (`ollama`, `opensearch`, `airflow`, `api`) akan jalan bersamaan untuk pertama kalinya
...
Sekarang trigger DAG `ingest_documents` — fungsi yang sama persis dengan yang dipanggil manual di Module 12-13 dan lewat form upload di Module 14, dipicu lewat Airflow sebagai cara **lain**. Ada dua cara — pilih salah satu.
...
Checklist ini melengkapi Checkpoint Praktik di Bagian 8 di atas — kalau kedua checklist sudah terverifikasi, Module 15 selesai.
...
- **Semua service terasa sangat lambat / laptop panas / container ter-*kill***: kemungkinan besar alokasi RAM Docker Desktop kurang — lihat catatan RAM di Module 6, bagian Panduan Praktik, dan naikkan ke 16GB+. Cek pemakaian resource real-time dengan `docker stats`.
