# Module 28: Polish Frontend untuk Demo Capstone

## Tujuan

Melakukan polish terakhir pada `chat.html`/`upload.html` (loading indicator, badge tool RAG-vs-SQL, cek responsif) supaya kemampuan backend yang sudah dibangun Module 7-25 terlihat jelas saat demo capstone, tanpa mengubah backend atau menambah framework baru.
...
`chat.html` dan `upload.html` sudah berfungsi penuh sejak Module 7 — Module 7 (chat dasar), Module 8 (streaming + multi-turn), Module 15 (upload) sudah membangun fondasinya, dan Module 17-25 menambah kemampuan **di belakang layar** (hybrid search, reranking, agent routing RAG-vs-SQL) tanpa pernah menyentuh HTML/CSS lagi. Efeknya: NALA sekarang jauh lebih pintar dari yang **terlihat** — dari sisi UI, jawaban agentic Module 21-25 tetap muncul sebagai teks polos yang tidak beda tampilannya dari jawaban RAG polos Module 7-16.

Module ini **bukan** membangun ulang frontend — ini pass polish terakhir sebelum capstone, dengan tiga target konkret:

1. **Tool-used badge** — staf yang mendemokan NALA (dan penilai capstone) bisa melihat sekilas apakah jawaban berasal dari RAG dokumen atau SQL tool (Module 21-25), tanpa harus buka Langfuse.
...
⚠️ **Asumsi yang perlu disesuaikan — dan hasil pengecekan nyata**: Module ini awalnya ditulis dengan asumsi endpoint agentic Module 21-25 mengembalikan metadata tool lewat baris JSON pertama di **stream**. Setelah dicek langsung ke implementasi nyata (Module 22 Bagian 3 Tahap C): routing RAG-vs-SQL (Module 21-25) **cuma terjadi di endpoint `/chat`** (mode Agent, non-streaming, dipicu checkbox "Pakai Agent" di `chat.html`) — `/chat/stream` **tetap RAG murni** sejak Module 7-16, sengaja tidak disentuh (keputusan desain eksplisit di Module 22). Jadi pendekatan streaming-metadata di rencana awal **tidak berlaku** untuk arsitektur ini — badge dipasang lewat field JSON biasa di response `/chat`, jauh lebih sederhana daripada parsing baris pertama stream. Kalau implementasi Anda berbeda (metadata tool memang ada di endpoint streaming), sesuaikan pendekatannya — tapi cek dulu endpoint mana yang benar-benar melakukan routing sebelum menulis kode, seperti yang kita lakukan di sini.
...
```css
/* app/static/style.css — tambahan Module 28 */
.typing-indicator {
    display: inline-flex;
    gap: 4px;
...
Karena routing RAG-vs-SQL cuma terjadi di `/chat` (non-streaming), pendekatannya jauh lebih sederhana dari rencana awal: tambahkan satu field ke `ChatResponse`, isi dari `called_tools` yang sudah dikembalikan `agent.invoke()` sejak Module 25 (RBAC) — tidak perlu endpoint tambahan atau parsing stream sama sekali.
...
Hanya tool yang **`diizinkan: True`** dihitung — kalau RBAC (Module 25) menolak percobaan SQL tool, itu tidak boleh muncul sebagai badge "Data Operasional (SQL)" karena datanya memang tidak pernah benar-benar diambil.
...
- called_tools sudah tersedia di final_state sejak Module 25
...
- Tidak ada perubahan pada logika agent/routing RAG-vs-SQL Module 21-25 (`tool_used` di Tahap B murni membaca hasil `called_tools` yang sudah ada, bukan mengubah cara agent memutuskan), atau pada `/health` — module ini murni lapisan tampilan di atas yang sudah ada.
