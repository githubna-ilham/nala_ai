# Module 20: Reranking dengan Cross-Encoder
...
- Sudah menyelesaikan **Module 19** — `Nala/` sudah punya `search_hybrid()` bekerja dan terhubung ke `/chat/stream`.
...
requirements.txt (Module 20, Langkah 1).
...
```
Siapkan volume cache HuggingFace untuk reranker (Module 20, Langkah 2).
...
(Module 20, Langkah 3) — belum dipakai endpoint apa pun.
...
- Kandidat yang dikirim ke rerank() nanti berasal dari
  vector_store.search_hybrid(top_k=20) (Module 19) — setiap dict
...
✅ **Indikator sukses**: tidak ada error, dan **urutan** hasil di dua bagian output kemungkinan besar berbeda — bukti cross-encoder menilai ulang relevansi, bukan sekadar mempertahankan urutan RRF. Perintah ini butuh waktu beberapa detik lebih lama dibanding `search_hybrid()` saja di Module 19 (20 pasangan query-dokumen dihitung satu per satu oleh cross-encoder) — ini normal, dibahas lebih lanjut di Bagian 6.
...
**Tidak satu pun** dari lima poin ini cocok dengan `### 2.1` di dokumen asli ("proposal pengajuan kredit", "surat keterangan domisili", "SIM" — semuanya tidak ada di SOP). Ini beda dari Module 19 Bagian 4, yang justru menghasilkan jawaban jujur "tidak ditemukan" untuk pertanyaan yang sama.
