# Module 28: Dashboard Monitoring & Guardrail/Security Review Checklist
...
- Dashboard Langfuse (`http://localhost:3000`) rutin dipakai untuk melihat latency, trace error, dan tool yang dipilih agent — bukan cuma untuk verifikasi instrumentasi seperti di Module 21.
...
NALA sekarang menjawab pertanyaan dari dokumen internal **dan** menjalankan query SQL ke data operasional PT Nusantara Finance (Module 22-26) — dua kanal akses data yang, kalau tidak diawasi, bisa jadi celah. Sebelum capstone, dua pertanyaan harus bisa dijawab dengan bukti, bukan asumsi:

1. **"Apakah kita tahu kalau sistem ini bermasalah?"** — Module ini menjawabnya lewat dashboard Langfuse (yang sudah menerima trace sejak Module 21) dan status page ringan yang menyatukan kesehatan semua service jadi satu tampilan.
2. **"Apakah kita sudah cukup hati-hati sebelum sistem ini dipakai staf finance sungguhan?"** — dijawab lewat checklist keamanan di Bagian 3, yang secara jujur memisahkan mana yang **sudah** diatasi sepanjang Module 1-26, dan mana yang **belum** — bukan daftar yang berpura-pura semua celah sudah tertutup.
...
### a. Dashboard Langfuse (sudah ada sejak Module 21, sekarang jadi bagian rutin operasional)

Langfuse (dinyalakan di Module 27 sebagai bagian stack, `http://localhost:3000`) sudah mencatat trace tiap kali `/chat` (agent, non-streaming, sejak Module 23) atau `/chat/stream` (streaming, sejak Module 8) dipanggil — ini bukan fitur baru Module 27-30, tapi Module 27-30 adalah pertama kalinya dashboard ini benar-benar dipakai untuk memantau, bukan cuma untuk memverifikasi instrumentasi (fokus Module 21).

Yang perlu dicek rutin dari dashboard Langfuse:

| Yang Dilihat | Kenapa Penting |
|---|---|
| **Latency per trace** | Kalau retrieval (OpenSearch) atau generation (Ollama) tiba-tiba melambat drastis, ini indikasi awal masalah infra (RAM penuh, container terlalu banyak berjalan bersamaan — lihat Module 27 Bagian 4) sebelum staf sempat komplain |
| **Trace yang error/exception** | Query SQL yang gagal (Module 22-26), OpenSearch tidak terjangkau (fallback Module 14 Bagian 2 Langkah 3) — semuanya seharusnya tercatat di Langfuse, bukan cuma di log container yang jarang dicek |
| **Tool apa yang dipilih agent** (RAG vs SQL, Module 22-26) | Kalau agent sering salah routing (misal pertanyaan SOP malah dijawab lewat SQL tool), ini sinyal prompt/routing logic Module 22-26 perlu ditinjau ulang |
...
**Prasyarat**: Module 27 sudah selesai — seluruh stack (`ollama`, `api`, `opensearch`, `opensearch-dashboards`, `airflow`, `postgres`, `adminer`, `langfuse`, `langfuse-db`) sudah jalan lewat `docker compose up -d` di `Nala/`, mengikuti Module 27 materi.md. Kalau Anda mematikan Langfuse sementara untuk menghemat RAM (lihat troubleshooting Module 27), nyalakan lagi sekarang sebelum lanjut — bukan pull/start dari awal:
...
```python
# app/main.py — tambahan di Module 28, setelah endpoint yang sudah ada
import httpx
...
- `postgres` diasumsikan sudah punya koneksi `engine`/`text` dari SQLAlchemy yang disiapkan Module 22-26 untuk SQL agent tool — sesuaikan nama variabel dengan implementasi Module 22-26 Anda sendiri kalau berbeda.
...
service NALA (Module 28, Langkah 1).
...
postgres (SELECT 1 lewat koneksi yang sudah ada dari Module 22-26) —
...
- Koneksi Postgres (engine/text SQLAlchemy atau setara, misal
  get_connection() psycopg) sudah dibuat Module 22-26 untuk SQL agent
...
`meta http-equiv="refresh"` dipakai sengaja alih-alih JavaScript polling — konsisten dengan prinsip frontend minimal NALA (server-rendered, tanpa build tooling) yang dipegang sejak Module 7-17.
...
untuk memantau kesehatan service NALA (Module 28, Langkah 2).
...
```
— tetap `200 OK`, tidak crash, persis sesuai target. Satu penyesuaian dari contoh materi: implementasi nyata memakai `get_connection()` (psycopg, role `nala_readonly` — sudah ada dari Module 24) untuk cek `postgres`, bukan `engine`/`text` SQLAlchemy seperti contoh ilustratif di atas — sesuaikan dengan pola koneksi database yang sudah dipakai di project Anda sendiri, jangan menambah dependency SQLAlchemy baru kalau belum pernah dipakai.
...
- [ ] **Tidak ada password/API key hardcoded** di `docker-compose.yml`, kode Python, atau file yang ter-commit ke Git — semua lewat `.env` (Module 27 Bagian 2 Tahap A) dan `.gitignore`.
...
**Status**: **sebagian** diatasi di Module 27 — bukan "selesai penuh", dan ini penting dicatat jujur. Yang sudah pindah ke `.env`: `POSTGRES_ADMIN_PASSWORD`, `LANGFUSE_DB_PASSWORD`, `LANGFUSE_NEXTAUTH_SECRET`, `LANGFUSE_SALT`. Yang **masih hardcoded** (diverifikasi langsung lewat `grep` ke `db/seed.sql`/`docker-compose.yml`): password role `nala_readonly`, `nala_writer`, `nala_app` (Module 24/25) — alasannya dijelaskan di Module 27 Bagian 4a (file `.sql` yang dijalankan `docker-entrypoint-initdb.d` tidak bisa baca `${VAR}` tanpa wrapper tambahan). Risikonya lebih rendah karena role-role ini cuma bisa diakses dari jaringan Docker internal, tapi tetap **bukan** praktik ideal untuk produksi sungguhan — dicatat di sini sebagai gap yang diketahui, bukan disembunyikan di balik klaim "`.env` sudah beres".

### b. RBAC & Audit Logging — Diterapkan End-to-End?

Module 26 membangun modul RBAC/audit logging (dilanjutkan dari `security.py`/`audit.py` di materi sumber). Pertanyaan reviewnya bukan "apakah modulnya ada", tapi **"apakah setiap jalur akses data benar-benar melewatinya"**:

- [ ] Endpoint SQL agent tool (Module 26) memverifikasi role/izin user **sebelum** menjalankan query — bukan cuma mencatat setelah query jalan.
...
- [ ] **Sudah ada** (Module 22-26, kalau RBAC diterapkan dengan benar): meskipun model "dibujuk" oleh teks jahat di dokumen, SQL agent tool tetap seharusnya menolak query yang melanggar RBAC di level tool — bukan bergantung pada model untuk menolak permintaan itu sendiri. Ini alasan kenapa validasi otorisasi **harus** ada di level tool/endpoint, bukan cuma di level prompt.
...
- [ ] Endpoint `/chat`, `/chat/stream`, `/upload` **tidak** punya rate limiting bawaan sejak Module 1-26 — ini celah nyata untuk deployment produksi (satu user/skrip bisa membanjiri Ollama dengan request, menghabiskan resource untuk user lain).
...
Yang perlu dipastikan sebelum lanjut ke Module 29:
...
Setelah semua poin di atas terpenuhi, lanjut ke **Module 29 materi.md, Tahap A — Loading/Typing Indicator** (`../Module-29-Polish-Frontend-Demo/materi.md`).

## Kesimpulan

Monitoring dan keamanan di module ini bukan fitur baru yang berdiri sendiri — keduanya adalah **lapisan verifikasi** di atas apa yang sudah dibangun sepanjang Module 1-26: Langfuse memverifikasi bahwa instrumentasi Module 21 benar-benar terpakai untuk mengawasi, status page menyatukan kesehatan tujuh+ service jadi satu pandangan, dan checklist keamanan memaksa kejujuran soal mana risiko yang sudah ditangani (grounding, RBAC, offline-first) dan mana yang masih terbuka (prompt injection dari dokumen, rate limiting). NALA yang siap didemokan bukan NALA yang mengklaim tidak punya celah — tapi NALA yang celahnya sudah dipetakan dan dikomunikasikan jujur, seperti yang akan diuji di rubrik capstone poin "pemahaman teknis trade-off".
