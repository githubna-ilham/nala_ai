...
- Ini docker-compose.yml paling awal project NALA — sengaja cuma
  ...
  sebelum service api ditambahkan (section 5).

GUARDRAIL:
- JANGAN tambahkan service api, build, depends_on, atau environment
  apa pun — itu semua baru masuk di section 5 (Tahap B).
...
- Service `ollama` di docker-compose.yml sudah ada dan sudah
  diverifikasi berjalan (section 3.4) — jangan diubah, cuma
...
> 📌 Langkah di bagian ini (Langkah 0-7) bernomor terpisah dari Langkah pembangunan kode `main.py` di section 5.2 di atas (Langkah 1-7, Tahap A-C) — keduanya adalah dua urutan "Langkah" yang berbeda dan tidak saling melanjutkan satu sama lain.
...
Ollama sudah terverifikasi sehat di Bagian A. Sekarang service `api` ditambahkan ke `docker-compose.yml` yang sama (lihat Tahap B di section 5.2 di atas untuk penjelasan lengkap blok `api` — `build: .`, `environment`, `depends_on`), dan kode `app/main.py` dibangun bertahap mengikuti section 5 Tahap A-C di atas.
...
- **Starter code**: `Nala/` — struktur project FastAPI app dan docker-compose NALA
