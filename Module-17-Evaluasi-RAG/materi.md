...
- `precision_at_k`: proporsi chunk relevan di antara `k` chunk teratas — dipanggil dengan `k=3` untuk meniru persis apa yang benar-benar dikirim ke LLM (`/chat/stream` mengirim top-3, lihat Module 15-16).
...
GOAL:
- Buat resources/starter-code/day-3/nala/app/evaluation.py berisi 4
  fungsi:
...
- **`before_rerank`**: memanggil `search_hybrid(..., top_k=3, ...)` langsung — persis alur Module 15 (tanpa reranking).
- **`after_rerank`**: memanggil `search_hybrid(..., top_k=20, ...)` untuk kandidat, lalu `reranker.rerank(..., top_k=3)` — persis alur Module 16.
...
(skrip perbandingan sebelum/sesudah reranking) — Module 17, Langkah
2-3.

GOAL:
1. Buat resources/starter-code/day-3/nala/app/eval_testset.py berisi
...
2. Buat resources/starter-code/day-3/nala/app/run_evaluation.py:
...
CONTEXT:
- app/evaluation.py (Langkah 1), app/vector_store.py (Module 15),
  app/reranker.py (Module 16) sudah ada.
...
Langkah eksekusi lengkap ada di bagian **Panduan Praktik** di bawah, Langkah 1-2. Yang perlu dipastikan sebelum lanjut ke Module 18:
...
## 9. Hasil Uji Nyata: Klaim Module 15-16 Terbukti dengan Angka
